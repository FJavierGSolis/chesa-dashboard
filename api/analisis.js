...
function VistaEjecutiva({ data, monthKey, funnelData, onIrADetalle }) {
  ...
  const generarBriefing = async () => {
    ...
    try {
      // 1. Datos operativos del mes actual + funnel
...
- Tasa plan piso: TIIE ${rawInv.tiie || 8.5}% + ${rawInv.spread || 2}% = ${((rawInv.tiie||8.5)+(rawInv.spread||2)).toFixed(2)}% anual`;
      ...
      // 4. Satisfacción cliente (Ventas, Servicio, Prospección del mes)
      let resumenSatisfaccion = "";
      const [rawSatV, rawSatS, rawSatP] = await Promise.all([
      ...
      // 5. Demos (encuestas públicas del mes)
      let resumenDemos = "";
      const rawDemos = await fbGet(`encuestasDemo/${monthKey}`);
      if (rawDemos) {
...
- Modelo más demostrado: ${topModelo ? `${topModelo[0]} (${topModelo[1]} demos)` : "—"}`;
      ...
      // 6. Productividad asesores (último trimestre)
      let resumenProductividad = "";
      const rawVentasHist = await fbGet(VENTAS_HIST_PATH);
...
- Asesor líder: ${ranking[0]?.[0] || "—"} con ${ranking[0]?.[1]?.ventas || 0} ventas`;
      ...
      // 7. Mercado ADACH (mes más reciente disponible)
      let resumenMercado = "";
      const rawAdach = await fbGet(ADACH_PATH);
